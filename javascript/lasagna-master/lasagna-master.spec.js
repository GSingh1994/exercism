import { cookingStatus, preparationTime, quantities, addSecretIngredient, scaleRecipe } from "./lasagna-master";
...
describe("preparationTime", () => {
  test("applies the custom average time per layer", () => {
    const manyLayers = ["sauce", "noodles", "béchamel", "meat", "mozzarella", "noodles", "ricotta", "eggplant", "béchamel", "noodles", "sauce", "mozzarella"];
...
describe("addSecretIngredient", () => {
  ...
  test("does not modify the first array", () => {
    const createFriendsList = () => ["noodles", "tomatoes", "sauce", "meat", "mozzarella", "eggplant", "ricotta", "parmesan"];
...
function expectObjectsToBeEqual(actualObj, expectedObj) {
  for (const key in expectedObj) {
    expect(actualObj[key]).toBeCloseTo(expectedObj[key], DIFFERENCE_PRECISION_IN_DIGITS);
  }
